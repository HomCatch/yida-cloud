...
public interface FilterInfoRepository extends JpaRepository<FilterInfo, Long> {
    ...
    @Query(value = "select filter_code from filter_info ORDER BY filter_code desc LIMIT 1" ,nativeQuery =  true)
    String maxCode();

    @Query(value = "insrt into filter_code (filter_code,status,inventory_time) values(?1,0,now())", nativeQuery = true)
    void addList(List<String> filterCodes);
}